...
class BTreeInnerNode<TKey extends Comparable<TKey>> extends BTreeNode<TKey> {
	protected static int INNERORDER = -1;
	...
	// Martha Rtree
	public void rprint() { // Printing helper
		try {
			this.rprintK();
		...
		while (this.children[i] != null) {
			((BTreeNode<TKey>) this.children[i]).rprint();
			i++;
		}
		System.out.print("]");
	...
	@SuppressWarnings("unchecked")
	@Override
	public void print() { // Printing helper
		this.printK();
		System.out.print("[");
		int i = 0;
		while (this.children[i] != null) {
			((BTreeNode<TKey>) this.children[i]).print();
			i++;
		}
		System.out.print("]");
